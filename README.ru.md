...
Содержимое `geosite` и наборов `.srs` проверить нельзя: это двоичные файлы,
которых у приложения нет. Если такой набор стоит выше найденного совпадения,
проверка честно об этом предупредит вместо того, чтобы выдать догадку.
