...
[![Релизы](https://img.shields.io/github/v/release/13CyberPunk02/CVPN?label=релиз)](https://github.com/13CyberPunk02/CVPN/releases)
[![Загрузки](https://img.shields.io/github/downloads/13CyberPunk02/CVPN/total?label=загрузок)](https://github.com/13CyberPunk02/CVPN/releases)
[![Тесты](https://github.com/13CyberPunk02/CVPN/actions/workflows/tests.yml/badge.svg)](https://github.com/13CyberPunk02/CVPN/actions/workflows/tests.yml)
...
Под индикатором на главном экране - сколько времени держится соединение,
сколько принято и отдано, какая была пиковая скорость и через какой сервер.
Данные берутся из того же потока счётчиков, что и текущая скорость,
отдельных запросов к ядру не делается.

Итоги остаются на экране после отключения: их для того и собирали.

## Язык интерфейса

Русский и английский, выбирается в настройках. По умолчанию берётся язык
системы.

Нейтральный язык ресурсов - английский: если перевода нет, показывается
английская строка, а не пустое место. Смена языка требует перезапуска -
приложение предложит его сразу.

Строки лога намеренно только на английском. Они попадают в файлы, которые
прикладывают к сообщениям об ошибках, и одноязычный лог читается всеми,
а не только автором.

Чтобы добавить язык, скопируйте `Localization/Strings.ru.resx`, переведите
значения и допишите код в `Loc.Available`. Подробности - в
[LOCALIZATION.md](LOCALIZATION.md).

## Значок в трее

Двойной клик открывает окно, меню даёт подключение, выход и - подменю
«Сервер» со списком профилей. Активный отмечен галочкой. Переключение идёт
через селектор, то есть без перезапуска ядра.
...
Поле над списком отвечает на вопрос, из-за которого обычно лезут в логи:
какое правило сработает. Вводите домен или целый адрес - приложение покажет
исход и назовёт сработавшее правило, не поднимая туннель.

Проверить можно не всё, и об этом сообщается прямо, а не догадкой:

- содержимое `geosite` и наборов `.srs` лежит в двоичных файлах, которых
  у приложения нет;
- `geoip` решается по адресу, в который резолвится домен, - базы соответствий
  у приложения тоже нет.

Если такое правило стоит выше найденного совпадения, проверка назовёт его
и объяснит, при каком условии сработает оно.
...
Рядом с каждой строкой кнопки. Первая предлагает противоположное текущему
выходу: для соединения через прокси - «Напрямую», для прямого - «Через прокси».
Вторая блокирует. Они добавляют правило `domain_suffix` для домена второго уровня - `sun9-40.vkuserphoto.ru` превратится
...
Локализация покрыта тоже: тест обходит весь код и проверяет, что каждый
использованный ключ есть в ресурсах, что русский перевод полон и что
подстановки вроде `{0}` совпадают в обоих языках. Это заменяет проверку
компилятором, которую дал бы сгенерированный класс ресурсов.
...
│   ├── ViewModels/          MainViewModel + вьюмодели страниц
...
Страницы постепенно переезжают на собственные вьюмодели: навигация подставляет
объект, а разметку WPF находит по `DataTemplate`. Так состояние страницы
переживает переходы. Шаблон переноса - в [REFACTORING.md](REFACTORING.md).
...
- [x] Проверка домена перед сохранением правила: показывать, какое правило сработает
- [x] Быстрое переключение сервера из меню в трее
- [x] Автоматическое обновление подписки раз в сутки
- [x] Статистика за сессию
- [x] Английский интерфейс
- [ ] Подпись установщика сертификатом

Предложения - в [issues](https://github.com/13CyberPunk02/CVPN/issues).
