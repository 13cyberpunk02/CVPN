...
public class ConfigBuilderTests
{
    ...
    /// <summary>
    /// Удалённый набор на старте ещё не скачан. Маршруты это переживают,
    /// а инициализация DNS-правил падает с «rule-set not found» - поэтому
    /// такие условия в DNS не переносятся.
    /// </summary>
    [Theory]
    [InlineData(MatchKind.Geosite, "youtube")]
    [InlineData(MatchKind.RuleSetRemote, "https://example.com/geosite-youtube.srs")]
    public void Удалённые_наборы_в_dns_правила_не_переносятся(MatchKind match, string value)
    {
        var routing = TestData.Routing(TestData.Rule(match, value, RouteAction.Direct));
        var config = TestData.BuildConfig(routing: routing);

        Assert.Equal(0, config.GetProperty("dns").GetProperty("rules").GetArrayLength());

        // при этом в маршрутах набор остаётся
        Assert.Equal(1, config.GetProperty("route").GetProperty("rule_set").GetArrayLength());
    }

    /// <summary>Локальный набор лежит на диске и доступен ядру сразу.</summary>
    [Fact]
    public void Локальный_набор_в_dns_правила_переносится()
    {
        var routing = TestData.Routing(
            TestData.Rule(MatchKind.RuleSetLocal, @"C:\rules\ru.srs", RouteAction.Direct));

        var config = TestData.BuildConfig(routing: routing);
        var rule = config.GetProperty("dns").GetProperty("rules").EnumerateArray().Single();

        Assert.Equal("dns-local", rule.Str("server"));
        Assert.Equal("ru", rule.GetProperty("rule_set").EnumerateArray().Single().GetString());
    }

    /// <summary>
    /// Главная проверка: каждый набор, на который ссылаются правила,
    /// обязан быть объявлен. Иначе ядро падает при запуске.
    /// </summary>
    [Fact]
    public void Все_упомянутые_наборы_объявлены()
    {
        var routing = TestData.Routing(
            TestData.Rule(MatchKind.Geosite, "youtube", RouteAction.Direct),
            TestData.Rule(MatchKind.RuleSetRemote, "https://example.com/custom-list.srs", RouteAction.Direct),
            TestData.Rule(MatchKind.RuleSetLocal, @"C:\rules\ru.srs", RouteAction.Direct),
        ...
        var declared = config.GetProperty("route").GetProperty("rule_set")
            .EnumerateArray().Select(s => s.Str("tag")).ToHashSet();
