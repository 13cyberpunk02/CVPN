...
///  • полный конфиг sing-box - берутся все прокси-outbound'ы;
...
public static class ConfigImporter
{
    public static bool TryImportFile(string path, out List<ServerProfile> profiles, out string error)
    {
        ...
        {
            error = Loc.T("Import_ReadFailed", ex.Message);
            return false;
        }
    }
    ...
    public static bool TryImport(string json, out List<ServerProfile> profiles, out string error)
    {
        ...
        {
            error = Loc.T("Import_NotJson", ex.Message);
            return false;
        }
        ...
        if (profiles.Count == 0)
        {
            error = Loc.T("Import_NoOutbounds");
            return false;
        }
    ...
    private static ServerProfile? FromOutbound(JsonElement e)
    {
        ...
        {
            Name = Str(e, "tag") is { Length: > 0 } tag ? tag : Str(e, "server") ?? Loc.T("Import_UnnamedProfile"),
            Host = Str(e, "server") ?? "",
