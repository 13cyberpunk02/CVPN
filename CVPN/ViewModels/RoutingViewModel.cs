...
public sealed class RoutingViewModel : PageViewModel
{
    ...
    public RoutingViewModel(MainViewModel shell) : base(shell)
    {
        ...
        RemoveRule = new RelayCommand(p => { if (p is RouteRule rr) Remove(rr); });
        MoveUp = new RelayCommand(p => { if (p is RouteRule rr) Move(rr, -1); });
        MoveDown = new RelayCommand(p => { if (p is RouteRule rr) Move(rr, +1); });
    ...
    public string TestInput { get => _testDomain; set => Set(ref _testDomain, value); }
    ...
    private void RunTest()
    {
        ...
        // Содержимое наборов нам недоступно, поэтому честно предупреждаем,
        // что какое-то из них могло перехватить домен раньше
        if (!match.IsCertain)
        {
            var names = match.Unknown.Select(r => $"{r.MatchLabel} {r.DisplayValue}");
 
            TestResult += $". Выше есть наборы, содержимое которых не проверить: " +
                          $"{string.Join(", ", names)} - если домен в одном из них, " +
                          "сработает оно.";
        }
    }
