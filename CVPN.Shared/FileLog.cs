...
public sealed class FileLog
{
    ...
    /// <summary>
    /// Общий экземпляр. До вызова Initialize пишет рядом с исполняемым файлом -
    /// так строки не теряются, даже если инициализацию забыли.
    /// </summary>
    public static FileLog Current { get; private set; } =
        new(Path.Combine(AppContext.BaseDirectory, "logs"));
    ...
    /// <summary>
    /// Вызывается один раз при старте: создаёт каталог и подчищает старое.
    /// Каталог передаётся снаружи - класс подключён и в приложение, и в службу,
    /// а пишут они в разные места.
    /// </summary>
    public static void Initialize(string directory)
    {
        Current = new FileLog(directory);
        Current.Prepare();
    }
